import numpy as np
import torch
from DuelingDQNNetwork import DuelingDQNNetwork
...
class DuelingDQNAgent(object):
    def __init__(self, gamma, eps, lr, n_actions, input_dims,
        ...
        self.eval = DuelingDQNNetwork(self.lr, self.n_actions, eval_name, self.input_dims, self.chkpt_dir)

        next_name = algo + "_" + env_name + "_next"
        self.next = DuelingDQNNetwork(self.lr, self.n_actions, next_name, self.input_dims, self.chkpt_dir)
    ...
    def epsilon_greedy(self, observation):
        if np.random.rand() > self.eps:
            ...
            _, actions = self.eval.forward(state)
    ...
    def learn(self):
        ...
        Vs, As = self.eval.forward(states)
        Vs_next, As_next = self.next.forward(next_states)
        Vs_eval, As_eval = self.eval.forward(next_states)
        pred = torch.add(Vs, (As - As.mean(dim=1, keepdim=True)))[indices, actions]
        next_p = torch.add(Vs_next, (As_next - As_next.mean(dim=1, keepdim=True)))
        eval_p = torch.add(Vs_eval, (As_eval - As_eval.mean(dim=1, keepdim=True)))
